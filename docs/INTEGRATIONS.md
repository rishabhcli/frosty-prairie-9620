...
This build runs entirely local-first (no cloud accounts, no spend) per the human entrant's
explicit decision on 2026-07-20, honoring [AGENTS.md](../AGENTS.md)'s human-approval boundary
("the human entrant approves cloud provisioning/cost... credentials... public deployment").
Every adapter below is written so activating the real backend is a **configuration change**,
not a rewrite — the interface each fixture implements is identical to its production sibling.
...
| CockroachDB Managed MCP | *(no local equivalent — Cloud-only feature; not implemented as a running adapter in this build)* | Not run. The Managed MCP endpoint only exists for CockroachDB Cloud clusters, and this build has none. | Point a Managed MCP client at the Cloud cluster's MCP endpoint (URL + OAuth/service-account key) | Requires a CockroachDB Cloud cluster (human-approved cloud provisioning) before it can be activated at all. `COCKROACH_MANAGED_MCP_ENDPOINT`/`_API_KEY` in `.env.example` are the placeholders for it. The qualifying CockroachDB feature actually demonstrated in this build is **Distributed Vector Indexing** (above), which is real and running. |
...
- **Fixture-mode by explicit, disclosed choice (labeled in the UI and the video's
  lower-thirds):** Bedrock planning (`BEDROCK_MODE=fixture` — no AWS credentials exist in
  this environment), vector embeddings (deterministic hash, not a trained model), and
  Managed MCP (Cloud-only feature, no Cloud cluster provisioned).
- **Never fabricated:** no sponsor API call, model output, or evaluation result in this
  repository is invented, replayed-and-relabeled-as-live, or asserted without the code path
  that produced it.
